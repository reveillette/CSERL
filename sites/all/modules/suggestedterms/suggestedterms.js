  ...
  attach: function (context) {
    ...
    $('a.suggestedterm').each ( function() {
      ...
      function toggleTermsInList() {
        ...
        var input_array = {};
        console.log($(termlist.closest('.form-type-textfield').find('input')));
        if (termlist.closest('.form-type-textfield').find('input').length > 0) {
          input_array = $.map($(termlist.closest('.form-type-textfield').find('input')).val().split(','), $.trim);
        } 
